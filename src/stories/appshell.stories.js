...
const Template = ({ type, navClick }) => {
    ...
    let drawerToggle = undefined;

    document.addEventListener('DOMContentLoaded', () => {
        //the event occurred
        drawer = document.getElementsByTagName('mwc-drawer')[0];
        const drawerToggle = document.getElementsByClassName('toggle-drawer')[0];
    }, { once : true });

    document.addEventListener('MDCTopAppBar:nav', (event) => {
  ...
  return `
    <mwc-drawer hasHeader type="${type}">
      <div slot="header">
        <mwc-icon-button class="toggle-drawer" onClick="" icon="menu"></mwc-icon-button>
        <svg class="logo-wordmark" style="width:120px" viewBox="0 0 696 133" fit=""  preserveAspectRatio="xMidYMid meet" focusable="false">
        ...
        <div style="display:flex; flex-direction:column; height:100%;">
            ...
            <div style="position:fixed; bottom:0; flex:1;">
        ...
        <div slot="appContent">
            ${topAppBarComponent({ title: 'App Shell Title'}, { parameters:{ fixed:true} })}
        </div>
    </mwc-drawer>`;
};
