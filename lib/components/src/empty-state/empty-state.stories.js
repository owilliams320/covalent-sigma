...
>>>>>>> 3f0d54e (cc-1/empty-state : refactored empty state component)
    }
  },
...
export const Page = ({ headline, subtitle, action, twoActions, icon }) => {
  ...
  <td-empty-state 
    ...
    icon="${icon}"
  >
    ${action && !twoActions ? '<td-button slot="buttons">Button Text</td-button>' : ''}
    ${twoActions ? 
...
// Card empty state
const Card = ({ cardOutlined, cardTitle, headline, subtitle, action, twoActions }) => {
  return `
    <td-card 
      cardTitle='${cardTitle}'
