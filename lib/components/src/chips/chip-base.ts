...
import { MDCChipAttributes, MDCChipCssClasses,  MDCChipEvents, MDCChipAnimation } from '@material/chips/chip/constants';
...
export class ChipBase extends BaseElement {
    ...
    */
    ...
    protected override render() {
        ...
        return html`
        <span class="${classMap(classes)}" role="${ this.filter ? 'presentation' : 'row' }" id="td-chip-${this.chipId}">
            <span class="mdc-evolution-chip__cell mdc-evolution-chip__cell--primary" role="${ this.filter ? 'presentation' : 'gridcell' }">
                <button class="mdc-evolution-chip__action mdc-evolution-chip__action--primary" role="${ this.filter ? 'option' : '' }" type="button" tabindex="-1">
    ...
    protected override firstUpdated() {
        ...
        const actionEls = this.mdcRoot.querySelectorAll('.mdc-evolution-chip__action');
        for (let i = 0; i < actionEls.length; i++) {
            const action = new MDCChipAction(actionEls[i]);
            this.actions.set(action.actionType(), action);
            actionEls[i].addEventListener(MDCChipActionEvents.INTERACTION, (event) => {
    ...
    protected renderGraphic () {
        const graphicContent = this.filter ?  this.renderCheckMark() : this.renderIcon();
    ...
    protected renderIcon (trailing = false) {
        return html`
            <span class="mdc-evolution-chip__icon mdc-evolution-chip__icon--${trailing ? 'trailing' : 'primary' }">
                ${this.icon}
            </span>`;
    }
    ...
    protected renderTrailing () {
        ...
        <span class="mdc-evolution-chip__cell mdc-evolution-chip__cell--trailing" role="gridcell">
            <button class="mdc-evolution-chip__action mdc-evolution-chip__action--trailing" type="button" tabindex="-1" data-mdc-deletable="true" aria-label="Remove ${this.label}">
    ...
    protected createAdapter(): MDCChipAdapter {
        return  {
            ...
            emitEvent: <D extends object>(eventName: MDCChipEvents, eventDetail: D) => this.dispatchEvent(new CustomEvent(eventName, { bubbles: true, detail: eventDetail })),
            ...
            getAttribute: (attrName: MDCChipAttributes) => this.mdcRoot.getAttribute(attrName),
            ...
            hasClass: (className: MDCChipCssClasses) => this.mdcRoot.classList.contains(className),
            /** Proxies to the MDCChipAction#isSelectable method. */
            isActionSelectable: (action: MDCChipActionType) => this.actions.get(action)?.isSelectable() ?? true,
            /** Proxies to the MDCChipAction#isSelected method. */
            isActionSelected: (action: MDCChipActionType) => this.actions.get(action)?.isSelected() ?? false,
            /** Proxies to the MDCChipAction#isFocusable method. */
            isActionFocusable: (action: MDCChipActionType) => this.actions.get(action)?.isFocusable() ?? true,
            /** Proxies to the MDCChipAction#isDisabled method. */
            isActionDisabled: (action: MDCChipActionType) => this.actions.get(action)?.isDisabled() ?? false,
            ...
            setActionDisabled: (action: MDCChipActionType, isDisabled: boolean) => this.actions.get(action)?.setDisabled(isDisabled),
            /** Proxies to the MDCChipAction#setFocus method. */
            setActionFocus: (action: MDCChipActionType, behavior: MDCChipActionFocusBehavior) => this.actions.get(action)?.setFocus(behavior),
            /** Proxies to the MDCChipAction#setSelected method. */
            setActionSelected: (action: MDCChipActionType, isSelected: boolean) => this.actions.get(action)?.setSelected(isSelected),
            /** Sets the style property to the given value. */
            setStyleProperty: (property: string, value: string) => this.mdcRoot.style.setProperty(property, value)
        }
    }
